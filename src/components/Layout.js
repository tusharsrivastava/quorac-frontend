...
const _Layout = (props) => {
  ...
  const { routes, pageTitle, showTitle } = props;
  ...
  const [ls, setls] = useState({ size: null, lg: "3", sm: "4", md: "4", extras: "" });
  const [rs, setrs] = useState({ size: "12", lg: "3", sm: null, md: null, extras: "" });
  const [ms, setms] = useState({ size: "12", lg: "6", sm: "7", md: "8", extras: "" });
  ...
  return (
    <>
      ...
      <Container>
        <Row className="my-4">
          {showTitle ? <h5 className="mb-4">{t(pageTitle)}</h5> : <></>}
          {hideLS ? (
            <></>
          ) : (
            <Col
              ...
              md={ls.md}
              sm={ls.sm}
              className={`order-2 order-sm-1 ${ls.extras}`}
            >
              <div className="d-flex flex-column">{leftSidebar}</div>
            </Col>
          )}
          <Col
            size={ms.size}
            ...
            md={ms.md}
            sm={ms.sm}
            className={`order-1 order-sm-2 ${ms.extras}`}
          >
            {children}
          </Col>
          {hideRS ? (
